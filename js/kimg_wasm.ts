...
function setActiveBindings(bindings: GeneratedBindingsModule, textBackend: boolean): void {
...
function initVariantSync(
  ...
  const preferredSimdBindings = textBackend ? textSimdBindings : simdBindings;
  const fallbackBindings = textBackend ? textBaselineBindings : baselineBindings;

  if (simdSupported()) {
    try {
      const output = preferredSimdBindings.initSync(input);
      setActiveBindings(preferredSimdBindings, textBackend);
      return output;
    } catch {
  ...
  setActiveBindings(fallbackBindings, textBackend);
  return output;
}
...
export default async function init(
  ...
  return initVariant(false, module_or_path);
}
...
export async function initText(
  ...
  return initVariant(true, module_or_path);
}
